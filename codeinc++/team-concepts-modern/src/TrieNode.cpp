...
TrieNode::TrieNode() : wordEnds(false), 
            english_alphabet(const_sizeof_english_alphabet, nullptr) {};
...
//TrieNode* TrieNode::at(const int index) {
  //  validate(index);
   ...
   // return  english_alphabet[index].get();
//}

SmartNode TrieNode::at(const int index) {
    validate(index);

    return  english_alphabet[index];
}

void TrieNode::emplace(int atIndex, SmartNode node) 
{
    if (atIndex > 0 && atIndex < 26) {
        english_alphabet[atIndex] = node;
    }
}

void TrieNode::populate(const int index, SmartNode alphabetNode) {
    validate(index);
